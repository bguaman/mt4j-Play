...
public class PhysicsScene extends AbstractScene {
	...
	public PhysicsScene(AbstractMTApplication mtApplication, String name) {
		...
		font = FontManager.getInstance().createFont(app, "arial.ttf", 3, MTColor.WHITE);
...
//		Vec2 gravity = new Vec2(0, 0);
		...
		for (int i = 0; i < 20; i++) {
			...
			if(i==0) paddleTex = mtApplication.loadImage(imagesPath + "carlos.png");
			else if(i==1) paddleTex = mtApplication.loadImage(imagesPath + "carla.png");
			else if (i==2) paddleTex = mtApplication.loadImage(imagesPath + "felix.png");
			else paddleTex = mtApplication.loadImage(imagesPath + "ball.png");
			
			c.setTexture(paddleTex);
		//Image im;

			
			//CTI-BG
			//ponemos texto a los circulos
			/*
			MTTextArea text = new MTTextArea(app, font);
			text.appendText("hi");
			text.setEnabled(false);
			text.setFillColor(new MTColor(0, 0, 0, 0));
			text.setStrokeColor(new MTColor(255, 0, 0, 0));
			//text.unregisterAllInputProcessors();
			c.addChild(text);
			//c.addChild(0, text);
			text.setPositionRelativeToParent(c.getCenterPointLocal());
			*/
